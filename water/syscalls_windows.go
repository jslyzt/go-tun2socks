...
func setStatus(fd syscall.Handle, status bool) error {
	...
	return syscall.DeviceIoControl(fd, tapIoctlSetMediaStatus, &code[0], uint32(4), &rdbbuf[0], uint32(len(rdbbuf)), &bytesReturned, nil)
}
...
func openDev(config Config) (ifce *Interface, err error) {
	...
	mac := make([]byte, 6)
	err = syscall.DeviceIoControl(file, tapWinIoctlGetMac, &mac[0], uint32(len(mac)), &mac[0], uint32(len(mac)), &bytesReturned, nil)
	if err != nil {
		return nil, err
	}
